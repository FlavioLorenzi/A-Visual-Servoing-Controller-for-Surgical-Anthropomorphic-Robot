...
% proximity gain correction vector
correction_vector = eye(1,6);
...
while h<=3
    ...
    if mode==1
        ...
        [~, state, detectedPoint, handle_seen , surface_norm ]=vrep.simxReadProximitySensor(clientID, h_PS, vrep.simx_opmode_streaming);
        ...
        err_vis= [us_d(1)-us(1); ...
          ...
          if mod(time,5)==0
              ...
              if(norm(detectedPoint) <= 0.02 )
                  % viene modificata solo se sotto i 2cm di distanza dal
                  % target
                  correction_vector = [1 1 1 0 0 0]*10^-1;
              end
...
%         end
        ...
        ee_displacement = (K*pinv(J)*err)*(correction_vector);
        disp(ee_displacement);
